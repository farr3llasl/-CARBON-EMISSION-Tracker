...
class MainWindow:
   ...
   def fetch_data(self):
       try:
           ...
           str_country_list = [str(element) for element in data['Country'].tolist()]
           str_country_list = sorted(set(str_country_list))
           del str_country_list[-1]
           self.country_1_dropdown['values'] = str_country_list
           #del self.country_1_dropdown['values'][-1]
           # print(data['Country'])
           self.country_2_dropdown['values'] = str_country_list
           #del self.country_2_dropdown['values'][-1]
           str_sector_list = sorted(set(data['Sector'].tolist()), key = data['Sector'].tolist().index)
           del str_sector_list[-1]
           self.sector_dropdown['values'] = str_sector_list
           #del self.sector_dropdown['values'][-1]
           # print(data['1970'])
           return data
